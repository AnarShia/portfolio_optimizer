...
opt_type = 1
risk_free_rate = 2.23 / 252
stocks = ['AAPL', 'ABNB', 'ADBE', 'ADI', 'ADP', 'ADSK', 'AEP', 'ALGN',
...
base_path = r"C:\Users\Muham\PycharmProjects\pythonProject\NASDAQ\\"
...
df = pd.read_csv(f"{base_path + stocks[0]}.csv", index_col='Date', parse_dates=True)
...
for stock in stocks:
    df = pd.read_csv(f"{base_path + stock}.csv", index_col='Date', parse_dates=True)
    df_tmp = df.reindex(all_dates, method='ffill')['Adj Close']
...
for period in range((
                            total_month // month_period) - 1):  # -1 eklenmesinin sebebi, son periyodun (9-12.ay) sadece eğitim için kullanılması. Total 3 periyot
    ...
    train_end_date = train_start_date + pd.DateOffset(months=month_period) + pd.DateOffset(
        days=1)  # 3 aylık eğitim sürecinin sonu "train_end_date" olur.
    if train_end_date > all_dates[
        -1]:  # Eğer eğitim periyodunun bitiş tarihi veri setinin son tarihinden daha sonraysa, bu periyoda atlanır.
        continue
    while not (
            train_end_date in all_dates):  # Eğer train_end_date belirtilen tarihlere sahip değilse, tarihi bir gün ileri alarak doğru tarihi bulmaya çalışır.
    ...
    test_end_date = test_start_date + pd.DateOffset(months=month_period) + pd.DateOffset(
        days=1)  # 3 aylık test süreci (eğitimin bittiği tarihten itibaren)
    if test_end_date > all_dates[
        -1]:  # Eğer hesaplanan test_end_date, veri setindeki son tarihten sonraysa, test bitiş indeksi -1 olarak atanır.
        test_end_index = -1
    ...
    period_dates.append(str(datetime.strptime(str(test_start_date), '%Y-%m-%d %H:%M:%S').date()) + " / " + str(
        datetime.strptime(str(test_end_date), '%Y-%m-%d %H:%M:%S').date()))
    ...
    [buyStocks, buyWeights] = trainPortfolio(prices, dates, opt_type,
                                             stocks)  # Eğitim seti üzerinde trainPortfolio fonksiyonunu çalıştırarak
    # en iyi hisseleri ve bu hisselere ne kadar yatırım yapılması gerektiğini hesapla.
    prices = close_prices.iloc[test_start_index: test_end_index][
        buyStocks]  # Test setindeki, eğitim setinde belirlenen hisselerin kapanış fiyatlarını al.
    final_budget = testPortfolio(prices, buyWeights, buyStocks,
                                 budget)  # Test setindeki hisselerin performansını testPortfolio fonksiyonuyla kontrol et ve
